"""E_cti.train.padp_for_test_server —— PADP policy 推理 server (真版)。

> 仿源端 PADP `serve_padp.py` 的设计:
>   1) 加载 ckpt(从 rollout 临时路径) + config(从 yaml)
...
**🚧 TODO (见 padp_for_test_interaction_TODO.md)**:
- 异步流水线(K 个 in-flight 请求)  ← 后续 TODO
- 消融:per-epoch 触发 reset vs per-rollout 触发 reset vs 不触发  ← 后续 TODO
"""
import argparse
import socket
import sys
...
def load_policy_from_ckpt(ckpt_path: str, config_path: str, device: str = "cuda:0"):
    """从 ckpt 还原 policy + normalizer(配置从 yaml 读)。

    🚧 TODO: 当前 policy 是 online(非 EMA)。若想用 EMA,需要 ckpt 同时 dump
    ema.averaged_model.state_dict() 或 server 启动时再走一次 EMA warmup。
    """
    logger.info("[server] Loading ckpt: %s", ckpt_path)
    ...
    logger.info("[server] Policy loaded: %s, shape_info=%s",
            ...
            while True:
                ...
                if msg_type == Msg.OBS:
                    ...
                    try:
                        ...
                        send_framed(conn, pack_error(f"obs_to_torch failed: {e}"))
                        continue
                    try:
                        with torch.no_grad():
                            out = policy.predict_action(obs_t)
                    except Exception as e:
                        logger.exception("[server] predict_action failed")
                        send_framed(conn, pack_error(f"predict_action failed: {e}"))
                        continue
                    ...
                    total_steps += 1
                    if total_steps % 50 == 0:
                        logger.info("[server] processed %d steps (last ep=%d step=%d, latency=%.1fms)",
