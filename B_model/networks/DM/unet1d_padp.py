...
源:抄自 PADP `diffusion_policy/model/diffusion/conditional_unet1d_padp.py` + `conv1d_components.py`,
   把 conv1d_components inline 进来(避免 B_model 跨文件依赖)。
   关键特征:time embedding 已被 PADP 后续版本移除(noise schedule 在 G_algo 处理),
   global_cond 直接来自 VM 的输出。
"""
import logging
...
# ============== 1D conv 组件(inlined from PADP conv1d_components.py) ==============
...
# ============== Conditional Residual Block(PADP 核心) ==============
...
class Unet1DPadp(nn.Module):
    ...
    sample:     [B, H, D_a]  (denoised action chunk)
    local_cond: 暂不用
    global_cond:[B, D_g]    (VM 输出 flatten)
    """
    ...
    def __init__(self, input_dim, local_cond_dim=None, global_cond_dim=None,
        ...
        if global_cond_dim is None:
            raise ValueError("global_cond_dim is required for PADP-style UNet")
        ...
        logger.info("Unet1DPadp built, params=%.3e", sum(p.numel() for p in self.parameters()))

    def forward(self, sample, local_cond=None, global_cond=None, **kwargs):
        """
        sample:     [B, T, D_a]
        local_cond: [B, T, D_l] or None
        ...
        if global_cond is None:
            global_feature = torch.zeros(sample.shape[0], 1, device=sample.device)
        else:
        ...
        for idx, (resnet, resnet2, upsample) in enumerate(self.up_modules):
            x = resnet(torch.cat([x, h.pop()], dim=1), global_feature)
            x = resnet2(x, global_feature)
            x = upsample(x)
